# Web SRE Release & Incident Playbook

A strategy and Claude Code prompt for release readiness review, post-release verification, and incident diagnosis on web production systems — with approval-gated probing, hypothesis-driven investigation, and explicit upstream service status checks before deep analysis.

---

## Strategy

**SRE work is a different shape from review work.** The review playbooks audit static artifacts against rubrics. SRE work is investigative: a release is in flight or has just shipped, something looks wrong (or might be), and the job is to *narrow down what changed*. The prompt has to support two distinct entry modes:

- **Pre-flight / release readiness review** — "we're about to release commit X, audit it." Static-ish, looks like the other playbooks.
- **Incident diagnosis** — "production is misbehaving, find why." Iterative, hypothesis-driven, needs different scaffolding.

One prompt covering both works if the first thing it does is *route* between them based on what the user supplies. Mashing them together without the route produces a prompt that does neither well.

**Approval-gated probing is the core posture.** The prompt should: enumerate what it *could* check, ask before it does, and treat each escalation (DNS lookup → headers → full page fetch → repeated polling) as a separate ask. This is also a safety property — an unattended SRE agent that decides to hammer a misbehaving production endpoint with retries makes the incident worse. The prompt forbids that explicitly.

**Discover access, don't assume it.** "All — discover what's available" means the prompt should look for: a `RUNBOOK.md` or `docs/runbooks/`, observability config (Datadog, Sentry, Grafana dashboards referenced in code), a status page URL, healthcheck endpoints declared in the repo, deployment config (AWS region, CDN, edge config), and any incident-response docs. Then it reports what it found and what it's missing, not silently proceeding without context.

**Stack bias toward TS/Go/AWS, but detect first.** When ambiguity arises, lean toward what your team uses (e.g., assume CloudWatch logs are reachable via AWS CLI rather than guessing GCP, prefer `pnpm`/`npm` over `yarn` unless evidence says otherwise, expect Lambda/ECS/CloudFront patterns). But Phase 1 still confirms — your team also runs Godot and Kotlin in places, and a stack mismatch is a strong signal something is genuinely off.

**Check upstream service status before app-level analysis.** This is the single highest-leverage probe and the easiest to skip. If AWS us-east-1 is degraded, your Stripe webhooks are queued, or your auth provider is having an incident, your hypothesis ranking against the codebase is worthless until you've ruled the upstream out. The classic "spent two hours debugging our code, it was DNS" pattern is what this prevents. The prompt makes it a gate, not a step — Phase 1 enumerates the dependencies, and Mode 3 will not proceed to hypothesis ranking until upstream health has been checked or explicitly skipped.
...
- **A — Confirmed findings & safe verifications.** Things proven by reading the repo or by approved probes. Includes "release looks clean against rubric."
- **B — Hypotheses with proposed next probes.** Each with the falsification cost, what it would prove, and an explicit ask to run.
- **C — Critical findings or active risks.** Live security exposures, data-loss risk, ongoing customer impact. Escalation language, not casual.

**One trap to encode.** Claude under-instructed will offer to "run a full health check" by hitting every endpoint it can find. During an incident this can compound the problem (caches filling with error responses, rate limits tripping, on-call alerts amplifying). The prompt requires that probes are minimal, sequential, and stopped at first signal.

**Mode switching.** The prompt's first job is to ask: are you doing release-readiness review, post-release verification, or incident diagnosis? Each routes to a different Phase 2.

---

## The Prompt

Paste into Claude Code, run from the repo root with the production URL(s) provided.
...
- The Phase 1 service dependency map is the foundation for everything downstream — if it's incomplete, the upstream check in Mode 3 Step 2 will miss the dependency that's actually degraded. Worth running Phase 1 alone once per repo and keeping the output as a living artifact in `docs/runbooks/dependencies.md` or similar, then feeding it into incident sessions rather than rebuilding it under pressure.
- Status pages move and rebrand. If the prompt marks one as "unknown," that's a prompt to add the canonical URL to the repo's runbook so future sessions don't need to ask.
- For repos already on AWS-heavy stacks, the dependency map will be long — that's the point. The cheap status sweep is what catches the case where it's actually one specific AWS service in one specific region degraded, not your code.
- The mode selection at the top is deliberately the first thing — don't let a hurried "production is on fire" message skip the framing step. The 30 seconds spent answering "Mode 3, here's the URL, started 10 min ago, ~5% of users affected" save many minutes of wrong-path investigation.

---

## Reference Material

- [Google SRE Book — chapter on Effective Troubleshooting](https://sre.google/sre-book/effective-troubleshooting/)
- [AWS Service Health Dashboard](https://health.aws.amazon.com/health/status)
- [GitHub Status](https://www.githubstatus.com/) · [npm Status](https://status.npmjs.org/) · [Stripe Status](https://status.stripe.com/) · [Cloudflare Status](https://www.cloudflarestatus.com/)
