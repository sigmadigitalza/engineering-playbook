---
name: repo-setup-review
description: Reviews or sets up a GitHub repo's configuration surface — branch and tag rulesets, CODEOWNERS, collaborator and admin roles, default GITHUB_TOKEN scope, Action SHA-pinning enforcement, secret scanning, contribution gates, and merge settings. Posture-aware (open-source / distribution-only / private-team / personal). Companion to github-actions-review. Use this whenever the user is setting up a new repo, auditing an existing repo's settings, configuring branch protection, adding CODEOWNERS, tightening rulesets, locking down a public repo, reviewing tag protection on a repo that publishes reusable workflows or actions via floating major tags, or asks 'how should this repo be configured' — even if they only say 'repo setup', 'lock this down', 'review repo settings', or 'harden this repo'.
---
...
This second question is independent of visibility and changes the approval-gate recommendation entirely. GitHub forbids users from submitting approving reviews on their own PRs, so `required_approving_review_count: ≥1` deadlocks a solo maintainer on their own work.
