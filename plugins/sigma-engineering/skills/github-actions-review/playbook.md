...
A strategy and Claude Code prompt for auditing GitHub Actions workflows for cost, performance, correctness, and security — with awareness of Blacksmith (a third-party GitHub-hosted-runner replacement vendor) runner adoption.

---

## Strategy

**Start from the blast radius.** A senior engineer reviews CI the way they review production access, because that's what it is: a workflow runs with the repo's secrets and write-scoped `GITHUB_TOKEN`, so one careless or malicious change — an unpinned action, a `pull_request_target` that checks out PR code, an over-broad `permissions:` block — can exfiltrate every secret or poison the releases that change ships. This is also the bar an AI agent reviewing or writing workflows is held to: the same supply-chain and secret-hygiene rules apply whether a human or an agent touched the YAML.
...
- **A — Functionality-preserving optimizations** (cache keys, adding `timeout-minutes`, removing an unnecessary `fetch-depth: 0`, path filters that provably don't change behavior). Only these get auto-applied.
