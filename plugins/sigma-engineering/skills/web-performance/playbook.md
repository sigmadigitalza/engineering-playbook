...
A strategy and Claude Code prompt for auditing web application performance against Core Web Vitals (LCP, INP, CLS), using Lighthouse and aligned with web.dev / MDN authoritative guidance. Mode-routed for both static code review and live site audit, with explicit quick-wins, lab-vs-field discipline, and approval-gated probing.
...
**Quick wins as a first-class section.** Performance has an unusually clear quick-win category. Adding `width`/`height` to images (CLS), `fetchpriority="high"` on the LCP image, `font-display: swap`, `preconnect` to third-party origins, removing `loading="lazy"` from above-the-fold images. Each is a 5-minute fix with a measurable effect. The prompt makes this its own bucket so they don't get buried in design-level recommendations.

**CWV-aligned organization for findings.** The web.dev guidance structures around LCP / INP / CLS, and that's the structure that maps best to debugging real-world issues. Organize findings by which CWV they affect, with cross-references where a single issue affects multiple metrics (e.g., a heavy third-party script hits both INP and LCP).

**The big counter-intuitive finding to encode.** Per recent web.dev data, the majority of poor-LCP origins spend *less than 10%* of their LCP time actually downloading the image. Most of the budget is burned on discoverability (image hidden behind JS, CSR-only, lazy-loaded incorrectly) and prioritization (browser doesn't know it's important). Devs default to "compress the image more" — usually wrong. The prompt biases toward checking discoverability before suggesting more compression.

**Specific traps to encode:**

1. **Lighthouse score chasing.** A 100 in lab can coexist with poor field performance. Optimize for CWV thresholds for real users, not for the Lighthouse number. Flag any team treating "we're at 95 in Lighthouse" as a finished state.

2. **Auditing every page individually.** Don't. Audit *templates* — homepage, product/article page, listing page, app shell. Most issues live in shared templates. The prompt asks which templates matter, not "give me all your URLs."

3. **CLS invisible in lab tests.** Lab tests don't scroll, don't interact, don't wait for late-loading ads or content. CLS in the lab can be 0 while field CLS is bad. The prompt explicitly checks for *known causes* of CLS (dimensionless images, swapping fonts, dynamically inserted DOM) rather than trusting lab CLS alone.

4. **INP from third-party scripts.** Most INP regressions come from third-party SDKs, not first-party code. Audit what's actually in `<script>` tags and what tag managers inject at runtime.

**Three buckets, calibrated for performance:**

- **A — Quick wins.** High-impact, low-effort, individually verifiable. May be auto-applied with approval.
- **B — Structural improvements.** Design discussions, refactors, framework choices. Follow-up prompt.
- **C — Critical regressions.** Failing CWV thresholds in field data, accidentally shipped multi-MB bundles, broken caching burning bandwidth right now. Escalation language.

**Probing posture.** Approval-gated like the SRE prompt. Running Lighthouse repeatedly against production has real cost — a single audit is fine; six audits across different network conditions is a deliberate measurement campaign that needs sign-off. PSI API calls are free but rate-limited.

---

## The Prompt

Paste into Claude Code, run from the repo root. Provide URL(s) for Mode 2.
...
- For sites with established CrUX data, lead with PageSpeed Insights — it gives both lab and field in one call. Lighthouse without CrUX is half the picture.
- The "audit templates not pages" rule is the most under-followed bit of advice in performance reviews. Pick three: homepage, primary content type, app shell. That's enough to find ~90% of issues.
- Field data wins ties. If lab says LCP is 1.8s and CrUX p75 says 4.2s, the real users are having a 4.2s experience. Spend effort on whatever's making real users slow, not on the lab number.
- Run alongside a bundle analyzer (`rollup-plugin-visualizer`, `webpack-bundle-analyzer`, `next-bundle-analyzer`) — the prompt is good at design and patterns; the bundle analyzer is good at "this dep is 400KB minified" specifics. Bring its output to the session.
- Re-measure after Section A is applied. Performance changes have variance — a single before/after run can mislead. Three runs or a Lighthouse CI assertion is a fairer check.

---

## Reference Material

- [web.dev — The most effective ways to improve Core Web Vitals](https://web.dev/articles/top-cwv) — the article that anchored this prompt.
...
- [Google Lighthouse on GitHub](https://github.com/GoogleChrome/lighthouse) · [Lighthouse CI](https://github.com/GoogleChrome/lighthouse-ci)
- [PageSpeed Insights](https://pagespeed.web.dev/) · [PSI API docs](https://developers.google.com/speed/docs/insights/v5/get-started)
- [HTTP Archive — Web Almanac](https://almanac.httparchive.org/) — yearly state-of-the-web data the web.dev recommendations cite.
