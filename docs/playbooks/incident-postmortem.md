...
> Conventions: `<…>` is a placeholder to fill in. `TK` marks content not yet supportable from the artifacts — the author confirms or supplies it before publishing.
...
## Contributing factors (and root cause, if singular)
...
- The action-item rubric is the single most load-bearing part of the system. A handful of action items that actually get done beats a long list that quietly decays. Defend the rubric — when the prompt rejects "improve communication during incidents," resist the temptation to bargain it back in.
